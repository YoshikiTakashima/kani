...
// keep everything that is not overwritten.
use crate::error::Error;
use crate::fmt;
use crate::ops::{Add, AddAssign, Sub, SubAssign};
use crate::sys_time as time;
...
impl SystemTime {
    ...
    pub fn checked_add(&self, duration: Duration) -> Option<SystemTime> {
        if let Some(sum) = self.0.checked_add_duration(&duration) {
            Some(SystemTime(sum))
        } else {
            None
        }
    ...
    pub fn checked_sub(&self, duration: Duration) -> Option<SystemTime> {
        if let Some(diff) = self.0.checked_sub_duration(&duration) {
            Some(SystemTime(diff))
        } else {
            None
        }
    }
