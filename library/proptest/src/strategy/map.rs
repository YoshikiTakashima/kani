...
impl<S: Clone, F> Clone for Map<S, F> {
    fn clone(&self) -> Self {
        Map { source: self.source.clone(), fun: Arc::clone(&self.fun) }
    }
}
...
impl<S: Strategy, O: fmt::Debug, F: Fn(S::Value) -> O> Strategy for Map<S, F> {
    ...
    fn new_tree(&self, runner: &mut TestRunner) -> NewTree<Self> {
        self.source.new_tree(runner).map(|v| Map { source: v, fun: Arc::clone(&self.fun) })
    }
}
...
//     fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
//         f.debug_struct("Perturb").field("source", &self.source).field("fun", &"<function>").finish()
//     }
// }

// impl<S: Clone, F> Clone for Perturb<S, F> {
//     fn clone(&self) -> Self {
//         Perturb { source: self.source.clone(), fun: Arc::clone(&self.fun) }
//     }
// }
